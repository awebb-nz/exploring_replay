...
class Tree:
    ...
    def init_params(self, root_belief, root_q_values, policy_temp, policy_type):
    ...
    def build_tree(self, horizon):
        ...
        for hi in range(1, self.horizon):
            ...
            if hi == 1:
                for a in range(2):
                    for r in [1, 0]:
                        b1 = self.belief_update(self.root_belief, a, r)
    ...
    def replay_updates(self, gamma, xi):
        ...
        evb_tree   = {hi:{} for hi in range(self.horizon)} # tree with evb values for each node
        qval_tree  = {hi:{} for hi in range(self.horizon)} # tree with value estimate for each node
            ...
            for k, b in self.tree[hi].items():
                ...
                for hin in reversed(range(hi)):
                    for kn, bn in self.tree[hin].items():
                        if kn[-1] == prev_c:
                            policy_proba = self.policy(qval_tree[hin][kn])
        ...
        while True: # for N steps
            ...
            for hi in reversed(range(self.horizon-1)):
                for k, b in self.tree[hi].items():
                    ...
                    v        = np.dot(self.policy(q), q)     # value of this belief state
                    ...
                    for a in range(2):
                        ...
                        for k1, q1 in qval_tree[hi+1].items():
                            ...
                            if prev_c == c and k1[0] == a:
                                v_primes += [np.dot(self.policy(q1), q1)] # values of next belief states
                        ...
                        v_new   = np.dot(self.policy(q_new), q_new) 
