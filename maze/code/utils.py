...
def plot_simulation(agent, data_path, save_path):
    ...
    for file in files:
        ...
        ax        = plt.subplot(111)
        plot_maze_values(Q_history[0], ax, num_y_states, num_x_states, goal_state, blocked_state_actions)
        ...
        for idx, Q_rep in enumerate(Q_history[1:]):
            fig       = plt.figure(figsize=(12, 5))
            ax        = plt.subplot(111)
            plot_maze_values(Q_rep, ax, num_y_states, num_x_states, goal_state, blocked_state_actions)
...
def plot_maze_values(Q, ax, num_y_states, num_x_states, goal_state, blocked_state_actions):
