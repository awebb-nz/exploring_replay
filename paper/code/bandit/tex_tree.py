...
def generate_value_diff_tree(qval_tree1, qval_tree2, file_path):
    ...
    with open(file_path, 'w') as f:
        ...
        for hi in range(1, h):
            ...
            for idx, y_node in enumerate(reversed(np.linspace(-y_max*(hi+1)/h,
                                                              y_max*(hi+1)/h,
                                                              num_nodes))):
                node_name = str(hi) + '_b_' + str(idx)
                f.write(r'\node[between] at (%.2f, %.2f) (%s){};' %
                        (x_node, y_node, node_name) + '\n')
        ...
        for hi in range(h):
            ...
            if hi == 0:
                ...
                y_node = 0
                f.write(r'\node[rectangle, text centered, draw=black, minimum height=1mm, text width=3mm, inner sep=0pt, fill=white, draw opacity=1] at (%.2f, %.2f) (%s){};' % 
                        (x_node, y_node, node_name) + '\n')
                state_nodes[hi].append(node_name)
            else:
                for idx, y_node in enumerate(reversed(np.linspace(-y_max*(hi+1)
                                                                  / h, y_max*(hi+1)/h,
                                                                  num_nodes))):
                    node_name = str(hi) + '_s_' + str(idx*2)
                    f.write(r'\node[rectangle, text centered, draw=black, minimum height=1mm, text width=3mm, inner sep=0pt, fill=white, draw opacity=1] at (%.2f, %.2f) (%s) {};' 
                            % (x_node, y_node+0.08, node_name) + '\n')
                    if hi == h-1:
                        qvals = diff_tree[hi][idx*2]
                        probas = np.exp(qvals)/np.sum(np.exp(qvals))
                        val = np.dot(probas, qvals) 
                        f.write(r'\node[qval] at (%.2f, %.2f) () {\tiny \textcolor{blue}{%.2f}};'
                                % (x_node+0.45, y_node+0.08, val) + '\n')
                    state_nodes[hi].append(node_name)
                ...
                for k1 in between_nodes[hi+1]:
                    ...
                    if cond:
                        ...
                        if q_val >= 0:
                            ...
                            qcolour = 'blue'
                        f.write(r'\draw[->, thick, %s] (%s) -- (%s) node [pos=0.80, above=-0.2em, sloped, font=\tiny] () {\textcolor{%s}{%.2f}};'
                                % (colour, k, k1, qcolour, q_val) + '\n')
...
def generate_big_tex_tree(tree, replays, q_history, need_history, file_path,
                          tree_height=None, between_height=0.1,
                          between_width=0.7, state_height=0.2,
                          state_width=0.45, gap=0.22):
    ...
    with open(file_path, 'w') as f:
        ...
        # f.write(r'\centering' + '\n')
        f.write(r'\begin{tikzpicture}' + '\n')
        f.write(r'\tikzstyle{between} = [rectangle, minimum height=%.2fcm, minimum width=%.2fcm, draw opacity=0]'
                % (between_height, between_width) + '\n')
        f.write(r'\tikzstyle{qval}    = [rectangle, text centered, text width=2cm]' + '\n')
        ...
        for hi in range(1, tree.horizon):
            ...
            for idx, y_node in enumerate(reversed(
                    np.linspace(-y_max*(hi+1)/tree.horizon,
                                y_max*(hi+1)/tree.horizon, num_nodes))):
        ...
        for hi in range(tree.horizon):
            ...
            if hi == 0:
                ...
                f.write(r'\node[rectangle, text centered, draw=black, minimum height=%.2fcm, minimum width=%.2fcm, fill=orange, fill opacity=%.2f, draw opacity=1, text opacity=1] at (%.2f, %.2f) (%s) {\tiny %.2f};'
                        % (state_height, state_width, alpha, x_node,
                           y_node, node_name, alpha) + '\n')
                state_nodes[hi].append(node_name)
            else:
                num_nodes = len(q_history[hi-1])*2
                for idx, y_node in enumerate(
                        reversed(np.linspace(-y_max*(hi+1)/tree.horizon,
                                             y_max*(hi+1)/tree.horizon, num_nodes))):
        ...
        for hi in range(tree.horizon-1):
            for k in state_nodes[hi]:
                ...
                for vals in tree.belief_tree[hi][idx1][1]:
                    ...
                    if text is None:
                        f.write(r'\draw[->, thick, %s] (%s) -- (%s) node [pos=0.70, above=-0.2em, sloped, font=\tiny] () {\textcolor{blue}{%.2f}};'
                                % (colour, k+'.east', k1+'.west', q_val) + '\n')
                    else:
                        f.write(r'\draw[->, thick, green] (%s) -- (%s) node [pos=0.35, above=-0.2em, sloped, font=\tiny] () {\textcolor{black}{%u}} node [pos=0.80, above=-0.2em, sloped, font=\tiny] () {\textcolor{blue}{%.2f}};'
                                % (k+'.east', k1+'.west', text, q_val) + '\n')
