...
def plot_root_values(data_folder):
    ...
    qval_history = np.load(os.path.join(data_folder,
                                        'replay_data',
                                        'qval_history.npy'),
                           allow_pickle=True)
    ...
    for i in [1, 2]:
        ...
        plt.axhline(v_full, linestyle='--', color='k', alpha=0.7,
                    label='Optimal value')
        ...
        if i == 1:
            ...
            plt.xticks(range(len(root_values)),
...
def plot_multiple(data_folder, M, P, R, nreps, R_true, horizons, xis, betas):
    ...
    fig, axes = plt.subplots(6, 2, figsize=(9, 18), dpi=100,
                             constrained_layout=True,
                             gridspec_kw={'height_ratios': [2, 2, 1, 2, 2, 1]})
    ...
    for hidx, h in enumerate(horizons):
        ...
        for bidx, beta in enumerate(betas):
            ...
            if bidx == (len(betas) - 1):
                ...
                print(hidx, R_true)
                axv.axhline(R_true[hidx], linestyle='--', color='k', alpha=0.7,
                            label='Optimal value')
                axp.axhline(R_true[hidx], linestyle='--', color='k', alpha=0.7,
                            label='Optimal value')
                ...
                axr.set_xticks(range(R.shape[2]),
                               ['%.4f' % i for i in xis[::-1]],
                               rotation=60,
                               fontsize=13)
    ...
    file_name = 'alpha0%u_beta0%u_alpha1%u_beta1%u_complete' % \
        (M[0, 0], M[0, 1], M[1, 0], M[1, 1])
    np.save(os.path.join(data_folder, file_name + '.npy'), R)
    plt.savefig(os.path.join(data_folder, file_name + '.svg'),
                transparent=True)
...
def analyse(data_path):
    ...
    for t in range(num_trees+1):
        ...
        replays = np.load(os.path.join(this_folder_seqs,
                                       'replay_data',
                                       'replay_history.npy'),
                          allow_pickle=True)
        num_seqs = 0
        ...
        replays = np.load(os.path.join(this_folder_noseqs,
                                       'replay_data',
                                       'replay_history.npy'),
                          allow_pickle=True)
    ...
    for idx, t in enumerate(idcs_greater):
        this_folder_noseqs = os.path.join(data_path, str(t), 'noseqs')
        q_init = np.load(os.path.join(this_folder_noseqs,
                                      'replay_data',
                                      'qval_history.npy'),
                         allow_pickle=True)[0]
        if idx == 0:
