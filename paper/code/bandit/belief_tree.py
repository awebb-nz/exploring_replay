...
class Tree:
                ...
                for next_idx in next_idcs:
                    ...
                    if self.arms[a] == 'known':
                        ...
                        if hi == self.horizon - 2:
                            v_primes = [self._value(qval_tree[hi+1][idx1]),
                                        self._value(qval_tree[hi+1][idx1])]
                        else:
                            v_primes = [eval_tree[hi+1][idx1],
                                        eval_tree[hi+1][idx1]]
                        ...
                        if hi == self.horizon - 2:
                            v_primes = [self._value(qval_tree[hi+1][idx1]),
                                        self._value(qval_tree[hi+1][idx2])]
                        else:
                            v_primes = [eval_tree[hi+1][idx1],
                                        eval_tree[hi+1][idx2]]
                    ...
                    eval_tree[hi][idx] += probs[a] * (b0*(1.0
                                                          + self.gamma*v_primes[0])
                                                            + b1*(0.0 + self.gamma*v_primes[1]))
    ...
    # def _find_belief(self, belief_tree, z):

    #     '''
    #     ---
    #     Check if the belief state z already exists in the 
    #     tree belief_tree
    #     ---
    #     '''

    #     for hi in range(len(belief_tree)-1):

    #         for idx, vals in belief_tree[hi].items():

    #             b = vals[0]

    #             if np.array_equal(b, z):
            ...
            for idx, vals in self.belief_tree[hi].items():
                ...
                for next_idx in next_idcs:
                    ...
                    if self.arms[a] == 'known':
                        ...
                        v_primes = [np.max(qval_tree[hi+1][idx1]),
                                    np.max(qval_tree[hi+1][idx1])]
                        ...
                        v_primes = [np.max(qval_tree[hi+1][idx1]),
                                    np.max(qval_tree[hi+1][idx2])]

                    b1 = 1 - b0

                    qval_tree[hi][idx][a] = b0*(1.0 +
                                                self.gamma*v_primes[0]) + \
                        b1*(0.0 + self.gamma*v_primes[1])
    ...
    def _find_belief(self, hi, z):
        ...
        for idx, vals in self.belief_tree[hi].items():
            ...
            if np.array_equal(np.array(b[0]), np.array(z[0])) and \
                    np.array_equal(np.array(b[1]), np.array(z[1])):
    ...
    def _generate_single_updates(self):
        ...
        for hi in reversed(range(self.horizon-1)):
            for idx, vals in self.belief_tree[hi].items():
                ...
                # current Q values of this belief state
                q = self.qval_tree[hi][idx].copy()
                b = vals[0]
                ...
                for next_idx in next_idcs:
                    ...
                    if self.arms[a] == 'known':
                        ...
                        idx1 = next_idx[1][0]
                        # values of next belief states
                        v_primes = [np.max(self.qval_tree[hi+1][idx1]),
                                    np.max(self.qval_tree[hi+1][idx1])]
                        ...
                        # values of next belief states
                        v_primes = [np.max(self.qval_tree[hi+1][idx1]),
                                    np.max(self.qval_tree[hi+1][idx2])]

                    b1 = 1 - b0

                    q_upd = b0*(1.0 + self.gamma*v_primes[0]) + \
                        b1*(0.0 + self.gamma*v_primes[1])
                    ...
                    updates += [[np.array(his),
                                 np.array(idcs),
                                 np.array(ays),
                                 q_news.copy(),
                                 np.array(gains),
                                 np.array(needs),
                                 np.array(np.cumsum(evbs))]]
                        ...
                        if prev_next_idx[0] == prev_a:
                            ...
                            for idx in [prev_idx1, prev_idx2]:
                                ...
                                for next_idx in next_idcs:
                                    ...
                                    v_primes = [np.max(self.qval_tree[prev_hi+2][idx1]),
                                                np.max(self.qval_tree[prev_hi+2][idx2])] # values of next belief states
    ...
    def replay_updates(self):
        ...
        while True:
            ...
            for idx, hi in enumerate(his):
                ...
                print('%u -- Replay %u/%u -- [%u, %u, %u], q_old: %.2f, q_new: %.2f, gain: %.3f, need: %.3f, evb: %.3f'
                      % (num, idx+1, len(his), hi, idcs[idx], aas[idx],
                         q_old[aas[idx]], q_new[aas[idx]], gains[idx],
                         needs[idx], evbs[idx]))
