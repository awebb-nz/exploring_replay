...
class Environment():
    ...
    def _get_new_state(self, s, a, unlocked=False):
        ...
        if (self._check_blocked([s, a]) is not None) and (not unlocked):
        ...
        if case1 or case2 or case3 or case4:
            ...
            # check barriers
            if unlocked or self._check_uncertain([s, a]) is None:
    ...
    def _convert_coords_to_state(self, coords: list):
        ...
        states = np.arange(self.num_states).reshape(
            self.num_y_states, self.num_x_states)
            ...
            for s in np.delete(range(self.num_states),
                ...
                for a in range(self.num_actions):
                    if ~np.isnan(Q_MB[s, a]):
                        ...
                        Q_MB_new[s, a] += \
                            (r + self.gamma*np.nanmax(Q_MB[s1, :]) -
                             Q_MB_new[s, a])
