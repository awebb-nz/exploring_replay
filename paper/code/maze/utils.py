...
def plot_env(ax, env):
            ...
            for bidx, l in enumerate(env.blocked_state_actions):
                if [st, ac] in l:
                    if env.barriers[bidx]:
                        i, j = np.argwhere(
                                           np.arange(env.num_states)
                                           .reshape(env.num_y_states,
                                                    env.num_x_states)== st).flatten()
                        if ac == 0:
                            ax.hlines((env.num_y_states-i),
                                      j, j+1, linewidth=6,
                                      color='b')
                        elif ac == 2:
                            ax.vlines(j, (env.num_y_states-i)-1,
                                      (env.num_y_states-i),
                                      linewidth=6, color='b')
                    break
    ...
    if len(env.nan_states) > 0:
        ...
        for s in env.nan_states:
            ...
            patches += [Rectangle((sx, env.num_y_states-sy-1), 1, 1,
    ...
    for (goal_y, goal_x) in env.goal_coords:
        ax.scatter(goal_x+0.5, env.num_y_states - goal_y - 0.5,
                   s=600, c='orange', marker=r'$\clubsuit$', alpha=0.7)
...
def add_patches(s, a, q, num_y_states, num_x_states):
    ...
    i, j = np.argwhere(np.arange(num_states)
                       .reshape(num_y_states, num_x_states) == s).flatten()
    
    # move up
    if a == 0:
        patches.append(RegularPolygon((0.5+j, num_y_states-0.18-i), 3,
                       radius=0.13, lw=0.5, orientation=0, edgecolor='k',
                       fill=True, facecolor=col_tuple))
    # move down
    elif a == 1:
        patches.append(RegularPolygon((0.5+j, num_y_states-0.82-i), 3,
                       radius=0.13, lw=0.5, orientation=np.pi, edgecolor='k',
                       fill=True, facecolor=col_tuple))
    # move left
    elif a == 2:
        patches.append(RegularPolygon((0.20+j, num_y_states-0.49-i), 3,
                       radius=0.13, lw=0.5, orientation=np.pi/2, edgecolor='k',
                       fill=True, facecolor=col_tuple))
    # move right
    else:
        patches.append(RegularPolygon((0.80+j, num_y_states-0.49-i), 3,
                       radius=0.13, lw=0.5, orientation=-np.pi/2, edgecolor='k',
                       fill=True, facecolor=col_tuple))
...
def plot_maze(ax, Q, agent, move=None, colorbar=True, colormap='Blues'):
    ...
    if np.all(Q_plot == 0):
        sns.heatmap(np.absolute(Q_plot), cmap=['white'], annot=False, fmt='.2f',
                    cbar=colorbar, vmin=0, vmax=1, ax=ax)
    else:
        sns.heatmap(np.absolute(Q_plot), cmap=colormap, annot=False, fmt='.2f',
                    cbar=colorbar, vmin=0, vmax=1, ax=ax)

    # arrows for actions
    patches = []
    for st in np.delete(range(agent.num_states),
                        agent.goal_states + agent.nan_states):
        for ac in range(4):
            if ~np.isnan(Q[st, ac]):
                if Q[st, ac] == 0:
                    patches += add_patches(st, ac, 0, agent.num_y_states,
                                           agent.num_x_states)
                else:
                    patches += add_patches(st, ac, Q[st, ac], agent.num_y_states,
                                           agent.num_x_states)
            for bidx, l in enumerate(agent.blocked_state_actions):
                if [st, ac] in l:
                    if agent.barriers[bidx]:
                        i, j = np.argwhere(np.arange(agent.num_states)
                                           .reshape(agent.num_y_states,
                                                    agent.num_x_states) == st).flatten()
                        if ac == 0:
                            ax.hlines((agent.num_y_states-i), j, j+1,
                                      linewidth=6, color='b')
                        elif ac == 2:
                            ax.vlines(j, (agent.num_y_states-i)-1,
                                      (agent.num_y_states-i), linewidth=6,
                                      color='b')
                    break
    ...
    if len(agent.nan_states) > 0:
        for s in agent.nan_states:
            ...
            patches += [Rectangle((sx, agent.num_y_states-sy-1), 1, 1,
                                  edgecolor='k', facecolor='k', linewidth=1)]
    ...
    for (goal_y, goal_x) in agent.goal_coords:
        ax.scatter(goal_x+0.5, agent.num_y_states - goal_y - 0.5,
                   s=600, c='orange', marker=r'$\clubsuit$', alpha=0.7)

    # agent location
    if move is not None:
        agent_state = move[-1]
        agent_y, agent_x = np.argwhere(np.arange(agent.num_states)
                                       .reshape(agent.num_y_states,
                                                agent.num_x_states)
                                       == agent_state).flatten()
        ax.scatter(agent_x+0.5, agent.num_y_states - agent_y - 0.5,
                   s=80, c='green', alpha=0.7)
...
def plot_need(ax, need, agent, colorbar=True, colormap='Blues',
    ...
    if np.all(need_plot == 0):
        sns.heatmap(need_plot, cmap=['white'], annot=False, fmt='.2f',
                    cbar=colorbar, ax=ax, vmin=vmin, vmax=vmax)
    else:
        sns.heatmap(need_plot, cmap=colormap, annot=True, fmt='.2f',
                    cbar=colorbar, ax=ax, vmin=vmin, vmax=vmax)
